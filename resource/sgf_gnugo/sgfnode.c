...
{
  void *pt = (void *) malloc(sizeof(char)*size);
...
{
  void *ptnew = (void *) realloc(pt, sizeof(char)*size);
...
sgf_write_header_reduced(SGFNode *root, int overwrite)
{
  ...
  if (overwrite || !sgfGetIntProperty(root, "AP", &dummy))
    sgfOverwriteProperty(root, "AP", "Gofu sgfparser:"VERSION);
